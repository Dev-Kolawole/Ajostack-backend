...
// 3. Initialize Services (Now they can safely see the .env variables)
...
// 🚨 THE MISSING AI DUNNING ROUTE 🚨
app.post('/api/treasury/remind', async (req, res) => {
  try {
    const { memberName, reason, amount } = req.body;

    // Change this line to use the active 2026 model
const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });

    const prompt = `You are a professional but firm cooperative treasurer for a Nigerian Ajo/esusu platform called AjoStack. 
    Write a short, direct WhatsApp message to ${memberName} reminding them that their automated monthly deduction of ₦${amount} failed. 
    The failure reason provided by the bank is: ${reason}. 
    Keep it strictly under 3 sentences. Be polite, localized, but demand immediate action to fund their account so the automated retry can clear.`;

    const result = await model.generateContent(prompt);
    const generatedMessage = result.response.text();

    res.status(200).json({ success: true, message: generatedMessage });

  } catch (error) {
    console.error("AI Engine Failure:", error);
    res.status(500).json({ success: false, error: "AI failed to generate message" });
  }
});
